...
/***************************************************************
Function: bool Maze::solveMaze()
Parameters:None
Returns:Bool
Notes:First method to call to solve maze, in this it calls private solveMaze method
***************************************************************/ 

bool Maze::solveMaze(){
	clearPath(); //Clear the path
	return solveMaze(start.xpos, start.ypos); //Call private clear method
}

/***************************************************************
Function:bool Maze::solveMaze(int row, int col)
Parameters:int row, int col
Returns:bool
Notes:A recursion method that keeps calling itself until a valid
path is found through the maze
***************************************************************/ 

bool Maze::solveMaze(int row, int col){
	if(maze[row][col].type == END) //If the tile is at an end space, return true
		return true;

	if(maze[row][col].type == WALL || maze[row][col].visited == true) //If the tile hits a WALL or already visited square, return false
		return false;

	maze[row][col].visited = true; //Mark tile a visited

	if(row != 0){
		if(solveMaze(row-1, col)){ //Go North, mark 'n' on stack
			path.push('n');
			return true;
		}
	}
	...
	if(row != validHeight){
		if(solveMaze(row+1, col)){ //Go South, mark 's' on stack
			path.push('s');
			return true;
		}
	}	
	...
	if(col != 0){
		if(solveMaze(row, col-1)){ //Go West, mark 'w' on stack
			path.push('w');
			return true;
		}
	}
	...
	if(col != validWidth){
		if(solveMaze(row, col+1)){ //Go East, mark 'e' pn stack
			path.push('e');
			return true;
		}
	}
	
	return false; //If it goes thorugh all if statements, most likely unsolveable maze return false
}

/***************************************************************
Function:void Maze MAze::printPath()
Parameters:None
Returns:Nothing
Notes:
***************************************************************/ 
...
void Maze::printPath(){
	while(!path.empty()){ //While stack is not empty
		cout << path.top() << " "; //Cout the top of stack
		path.pop(); //pop the top of stack
	}
}
